            ...
            VStack{
                ZStack {
                    if colorScheme == .light {
                        article.lightImage.resizable().scaledToFill().frame(width: UIScreen.main.bounds.width, height: 250).clipped().edgesIgnoringSafeArea(.top)
                    }
                    else {
                        article.darkImage.resizable().scaledToFill().frame(width: UIScreen.main.bounds.width, height: 250).clipped()
                    }
                    
                    Button(action: {
                        withAnimation(.easeInOut(duration: 0.3)) { isArticleOpened.toggle() }
                    }) {
                        Image(systemName: "xmark.circle").resizable().frame(width: 20, height: 20).foregroundColor(colorScheme == .dark ? .black : .white)
