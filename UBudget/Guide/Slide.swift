...
struct Slide: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                Image(imageOptions[index]).resizable().frame(width: UIScreen.main.bounds.width - 40, height: UIScreen.main.bounds.width - 40)
                Text(textOptions[index]).font(Font.custom("DIN", size: 20)).multilineTextAlignment(.center).padding()
            }
        }.gesture(
