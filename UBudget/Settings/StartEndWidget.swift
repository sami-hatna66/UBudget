...
struct StartEndWidget: View {
    ...
    var body: some View {
        VStack (spacing: 3) {
            HStack (spacing: 0) {
                Text("Start: ").foregroundColor(.white).font(Font.custom("DIN", size: 18)).padding(.leading, 10)
            ...
            HStack (spacing: 0) {
                Text("End: ").foregroundColor(.white).font(Font.custom("DIN", size: 18)).padding(.leading, 10)
        ...
        .background(
            RoundedRectangle(cornerRadius: 10).fill(Color.black).frame(width: UIScreen.main.bounds.width - 30).overlay(
