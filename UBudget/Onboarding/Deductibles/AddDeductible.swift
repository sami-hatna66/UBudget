...
struct AddDeductible: View {
    ...
    var body: some View {
        VStack {
            ...
            VStack (alignment: .leading) {
                ...
                HStack {
                    ...
                    Button(action: {
                        if name != "" {
                            deductibleCollection.append(
                                DeductibleStruct(name: name, amount: Double(amount) ?? 1, interval: selectedInterval)
                            )
                            showingSheet = false
                        }
                    }) {
        ...
        .background(Color(red: 36/255, green: 36/255, blue: 37/255, opacity: 1.0).edgesIgnoringSafeArea(.all))
    }
}
